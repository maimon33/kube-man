...
function bootstrapUrl(path: string) {
  return `${window.location.protocol}//${window.location.hostname}:7683${path}`;
}

export default function BootstrapGate({ user }: { user: User }) {
  ...
  if (!session.authenticated || !session.user) return <GoogleSignIn adminEmail={status.adminEmail || user.email} />;
...
function BootstrapSetup({ builtInGoogleAvailable, complete }: { builtInGoogleAvailable: boolean; complete: (status: BootstrapStatus) => void }) {
  ...
  return <><main className="bootstrap-page">
    <section className="bootstrap-intro">
      ...
      <div className="bootstrap-copy"><span>First-run setup</span><h1>Secure the control plane before connecting your clusters.</h1><p>A one-time startup code proves you have access to this Mac and its Docker environment. Then configure Google sign-in and establish the first administrator.</p></div>
      <div className="bootstrap-assurances"><div><i>1</i><span><strong>Local possession check</strong><small>The startup code is generated inside Docker and expires permanently after setup.</small></span></div><div><i>2</i><span><strong>Google email identity</strong><small>Google passes a verified email to KubeMan so it can match and authenticate users.</small></span></div><div><i>3</i><span><strong>First administrator</strong><small>The initial email becomes the system owner and can invite other users.</small></span></div></div>
      <small className="bootstrap-local">Local-only bootstrap service · localhost access</small>
    </section>
